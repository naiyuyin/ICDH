...
def simulate_linear_neq(W, W_0, B, B_0, n):
    ...
    for j in ordered_vertices:
        parents = G.neighbors(j, mode=ig.IN)
        X[:, j] = _simulate_single_equation(X[:, parents], W[parents, j], W_0[j], B[parents, j], B_0[j])
    return X


def simulate_linear_neq_v2(W, W_0, A, B, n):
    def _simulate_single_equation(X, w, w0, a, b):
        """X: [n, num of parents], w: [num of parents], x: [n]"""
        e = np.random.normal(loc=0, scale=np.sqrt(np.random.uniform(a, b, size=(n,))), size=(n,))
        x = X @ w + w0 + e
        return x
    ...
    for j in ordered_vertices:
        parents = G.neighbors(j, mode=ig.IN)
        X[:, j] = _simulate_single_equation(X[:, parents], W[parents, j], W_0[j], A[j], B[j])
    return X
