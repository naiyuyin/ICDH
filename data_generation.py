...
def main(args):
    ...
    path = f"data/{data_type}_poly"
    file_dir = f"data/{data_type}_poly/{graph_type}{s0}_d{d}"
    data_dir = f"data/{data_type}_poly/{graph_type}{s0}_d{d}/data"
    graph_dir = f"data/{data_type}_poly/{graph_type}{s0}_d{d}/graph"
    ...
    for t in range(10):
        if data_type == "hetero":
            ...
            X, A, A_0, B, B_0, G = hetero_old_synthetic_data_generation(d, n, s0 * d, graph_type, A_range, a_range,b_range)
            np.savetxt(os.path.join(data_dir, f"X_{t + 1}.txt"), X, delimiter=",")
            np.savetxt(os.path.join(graph_dir, f"A_{t + 1}.txt"), A, delimiter=",")
