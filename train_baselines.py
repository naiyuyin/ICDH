...
def train_synthetic_notears(args):
    ...
    if not os.path.isdir(f"results/{data_type}"):
        os.mkdir(f"results/{data_type}")
    ...
    for iter in range(10):
        ...
        end_time = time.time()
        res[iter, 4] = end_time - start_time
        np.savetxt(os.path.join(res_dir, f"A_notears_{iter + 1}.txt"), A_notears, delimiter=",")


        A_notears[np.abs(A_notears) < 0.3] = 0
        SHD, extra, missing, reverse = ut.count_accuracy(A_gt, A_notears != 0)  # SHD with thres as 0.3
        res[iter, 0] = SHD
        res[iter, 1] = extra
        res[iter, 2] = missing
        res[iter, 3] = reverse

        print(f"graph {iter + 1}: SHD(0.3): {SHD}, extra: {extra}, missing: {missing}, reverse: {reverse}, runtime: {res[iter, 1]: .4f}.")
        print("------------------------------ Done ------------------------------\n\n\n")
        np.savetxt(os.path.join(res_dir, "notears_res.txt"), res, delimiter=",")
    mean_res = np.mean(res, axis=0)
    std_res = np.std(res, axis=0)
    print(f"------------------------------ NOTEARS Results Summary: {graph_type}{s0} d{d} ------------------------------")
    print(f"MEAN SHD(0.3): {mean_res[0]: .1f} +/- {std_res[0]: .2f}\nMEAN EXTRA: {mean_res[1]: .1f} +/- {std_res[1]: .2f}\nMEAN MISSING: {mean_res[2]: .1f} +/- {std_res[2]: .2f}\nMEAN REVERSE: {mean_res[3]: .1f} +/- {std_res[3]: .2f}\nMEAN TIME: {mean_res[4]: .1f} +/- {std_res[4]: .2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_type', type=str, default='homo', choices=['homo', 'hetero', 'homo_nv'], help='data type')
