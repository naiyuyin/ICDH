...
def train_realdata():
    ...
    # print(A_notears)
    end_time = time.time()
    print("Runtime for NOTEARS is %f."%(end_time - start_time))
    np.savetxt("A_notears.txt", A_notears, delimiter=",")
    print("Apply our formulation.")
    A, B, B0, var_est, losses, t_nll = NLL_linear(X=X)
    print("Runtime for NLL is %f."%(t_nll))
    np.savetxt("A_nll.txt", A, delimiter=",")
