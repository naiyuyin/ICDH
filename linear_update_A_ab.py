...
def nll_expected(x, A, a, b):
    ...
    """
    r = x - x @ A
    return 0.5 / M * (((np.log(a + b + epsilon) - np.log(a + epsilon)) / (b + epsilon)) * (r ** 2)).sum() \
           + 0.5 * (((a + b) * np.log(a + b + epsilon) - a * np.log(a + epsilon)) / (b + epsilon)).sum() \
           + N * 0.5 * (np.log(2 * np.pi) - 1)
...
def update_A(X,
    ...
    def _func_A(params):
        ...
        # print(0.5 * (((np.log(a + b) - np.log(a)) / b) * (R ** 2)).sum())
        # print(0.5 * M * (((a + b) * np.log(a + b) - a * np.log(a)) / b).sum())
        # print(M * N * 0.5 * (np.log(2 * np.pi) - 1))
        loss = 0.5 * (((np.log(a + b) - np.log(a)) / b) * (R ** 2)).sum() \
               + 0.5 * M * (((a + b) * np.log(a + b) - a * np.log(a)) / b).sum() \
               + M * N * 0.5 * (np.log(2 * np.pi) - 1)
        h, G_h = _h(A)
        ...
        # G_loss_A = ((np.log(a + b + epsilon) - np.log(a + epsilon)) / (b + epsilon) / M ) * (- X.T @ R)
...
def update_a_b(X,
    ...
    def _fun_a_b(params):
        ...
        # G_loss_a = 0.5 * (np.log(a + b + epsilon) - np.log(a + epsilon)) / (b + epsilon) \
        #            - 0.5 / M * (R ** 2).sum(axis=0) / ((a + b) * a + epsilon)
        # G_loss_b = 0.5 * (b + a * (np.log(a + epsilon) - np.log(a + b + epsilon))) / (b ** 2 + epsilon) + \
        #            0.5 / M * (R ** 2).sum(axis=0) * \
...
def nll_linear_A_a_b(X,
    ...
    if data_type == "hetero":
        # a_est = np.random.uniform(0.1, 0.5, size=(1, N))
        # b_est = np.random.uniform(0.1, 0.5, size=(1, N))
        a_est = np.ones([1, N])
        b_est = np.ones([1, N])
        # a_est = np.ones([1, N]) * np.random.uniform(0.1, 0.5)
        # b_est = np.ones([1, N]) * np.random.uniform(0.1, 0.5)
    elif data_type == "homo":
        # a_est = np.ones([1, N])
        # b_est = np.ones([1, N])
        a_est = np.ones([1, N]) * np.random.uniform(0.1, 0.5)
