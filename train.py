...
from linear_update_A_a import nll_linear_A_A as nll_linear_A
import notears
...
def train_synthetic_data(args):
    ...
    res_dir = f"results/{data_type}/{graph_type}{s0}_d{d}_n{n}_lamb{args.lamb_a}"
    ...
    for iter in range(10):
        ...
        if data_type == "homo_nv":
            X = np.loadtxt(os.path.join(f"data/{data_type}/{graph_type}{s0}_d{d}", f"X_{iter + 1}.txt"), delimiter=",")
            A_gt = np.loadtxt(os.path.join(f"data/{data_type}/{graph_type}{s0}_d{d}", f"W_{iter + 1}.txt"),
                          delimiter=",")
        else:
        ...
        if args.formulation == 2:
            ...
            #                                                                 lamb=args.lamb_a,
            #                                                                 verbose=True)
            A_nll, B0_nll, var_est, nll, rec, t_nll = nll_linear(X=X,
                                                                   A_gt=A_gt,
                                                                   lamb=args.lamb_a,
                                                                   verbose=True)
        ...
        if args.formulation == 2:
            ...
            np.savetxt(os.path.join(res_dir, f"A_formulation{args.formulation}_nll_{iter+1}_lamb{args.lamb_a}.txt"), A_nll, delimiter=",")
            # np.savetxt(os.path.join(res_dir, f"B_formulation{args.formulation}_nll_{iter + 1}_lamb{args.lamb_a}.txt"), B_nll, delimiter=",")
            np.savetxt(os.path.join(res_dir, f"B0_formulation{args.formulation}_nll_{iter + 1}_lamb{args.lamb_a}.txt"), B0_nll, delimiter=",")
            np.savetxt(os.path.join(res_dir, f"var_est_{iter + 1}_lamb{args.lamb_a}.txt"), var_est, delimiter=",")
        ...
        print(f"graph {iter + 1}: SHD(0.3): {SHD}, uncertainty: {res[iter, 4]: .4f}, nll loss: {nll: .4f}, rec loss: {rec: .4f}, runtime: {t_nll: .4f}.")
        print("------------------------------ Done ------------------------------")
        np.savetxt(os.path.join(res_dir, "nll_res.txt"), res, delimiter=",")
    mean_res = np.mean(res, axis=0)
    std_res = np.std(res, axis=0)
    print(f"------------------------------ Results Summary: {graph_type}{s0} d{d} lamb {args.lamb_a} ------------------------------")
    print(f"MEAN SHD(0.3): {mean_res[0]: .1f} +/- {std_res[0]: .2f}\t MEAN UNCERTAINTY: {mean_res[4]: .1f} +/- {std_res[4]: .2f}\t MEAN TIME: {mean_res[1]: .1f} +/- {std_res[1]: .2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_type', type=str, default='hetero', choices=['homo', 'hetero', 'homo_nv'], help='data type')
