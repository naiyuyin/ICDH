...
def main_our(args):
    ...
    for s in range(10):
        ...
        start_time = t.time()
        # print(device)
        model = MLP(dims=[d, 10, 1], device=device, bias=True)
        W_est, nll = Nonlinear_update(model=model, X=X, lamb1=args.lamb1, lamb2=args.lamb2, device=device, W_true=W_true) # no l1 and l2 constraint version
        end_time = t.time()
...
def main_syn(args):
    ...
    for s in range(10):
        ...
        if args.data_type == 'homo_ev':
            ...
            np.savetxt(data_dir + f"/X_{s + 1}.csv", X, delimiter=',')
        elif args.data_type == 'hetero':
            X, var_est = ut.simulate_nonlinear_sem_hetero(W_true, n, sem_type)
            np.savetxt(data_dir + f"/X_{s + 1}.csv", X, delimiter=',')
            np.savetxt(graph_dir + f"/var_{s+1}.csv", var_est, delimiter=',')
    # W_true = ut.simulate_dag(d, s0, graph_type)
